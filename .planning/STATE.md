...
branch: fix/durable-recipe-thumbnails
status: active
...
> Scanned 2026-07-01 from code + git + data model. Replaces the old GSD-format
...
- **Media storage** — public Supabase Storage bucket `recipe-thumbnails`; backend copies expiring social CDN thumbnails before persistence.
...
On `fix/durable-recipe-thumbnails` — replacing expiring Instagram/TikTok CDN
thumbnail URLs with application-owned Supabase Storage URLs and backfilling
existing recipes.
...
### Post-v1.1 work (Mar 14 → Jul 1, not phase-tracked)
...
- **RLS hardening:** RLS enabled on `ingredients`, `workspaces`, `workspace_users`; workspace create/join/leave/count moved to `private` schema SECURITY DEFINER fns with public SECURITY INVOKER wrappers; `is_workspace_member` helper in `private`
- Durable recipe thumbnails: social CDN images are validated and copied into Supabase Storage; existing non-null thumbnails backfilled.
...
- Storage bucket `recipe-thumbnails` — public JPEG/PNG/WebP recipe previews, 8 MiB object limit
...
Finish durable-thumbnail branch → PR. The local migration is idempotent; linked
CLI migration history still needs a successful authenticated `supabase db push`.
